...
import tmpl from './otbaLWCOuterSensoryColdHeat.html';
import OmniscriptFormula from 'omnistudio/omniscriptFormula';
import { OmniscriptBaseMixin } from 'omnistudio/omniscriptBaseMixin';

export default class OtbaLWCOuterSensoryColdHeat extends OmniscriptBaseMixin(OmniscriptFormula) {
    circles = [                
        ...
        if (this.jsonData && this.jsonData.PhysicalAssessment && this.jsonData.PhysicalAssessment.HC_CHA_OTBA_SensoryObservationColdHeatsensation) {
            ...
            this.bodyPartSelections = BodyPartAssessmentData
                ...
                .map(item => ({
                    ...
                    HC_CHA_OTBA_ColdHeatsensationfinding: item.HC_CHA_OTBA_ColdHeatsensationfinding,
                    HC_CHA_OTBA_SpecifyColdHeatsensation:item.HC_CHA_OTBA_SpecifyColdHeatsensation                    
                }));
        }        
        
        if(this.jsonData && this.jsonData.PhysicalAssessment && this.jsonData.PhysicalAssessment.Block24 && this.jsonData.PhysicalAssessment.Block24.HC_CHA_OTBA_LocationofColdHeatSensationFormulaResult)
        {
            const formulaValue = this.jsonData.PhysicalAssessment.Block24.HC_CHA_OTBA_LocationofColdHeatSensationFormulaResult;
            if (formulaValue==1) {            
    ...
    isCircleBlue(circle) {
        return this.bodyPartSelections.some(selection => selection.HC_CHA_OTBA_LocationofColdHeatsensation === circle['data-value']);
    }   
        ...
        this.circles.forEach(circle => {
            if (circle.selected) {
                ...
                if (isTooltip && this.tooltipVisible && this.tooltipText === circle['data-value']) {
                    ...
                    const tooltipProperties1 = `Cold/Heat sensation finding  ${matchedSelection.HC_CHA_OTBA_ColdHeatsensationfinding}`;
                    const tooltipProperties2 = `Specify Cold/Heat sensation  ${matchedSelection.HC_CHA_OTBA_SpecifyColdHeatsensation}`;
