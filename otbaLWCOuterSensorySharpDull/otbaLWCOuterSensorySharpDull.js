...
import tmpl from './otbaLWCOuterSensorySharpDull.html';
import OmniscriptFormula from 'omnistudio/omniscriptFormula';
import { OmniscriptBaseMixin } from 'omnistudio/omniscriptBaseMixin';

export default class OtbaLWCOuterSensorySharpDull extends OmniscriptBaseMixin(OmniscriptFormula) {
    circles = [                
    ...
    showTheResult(){        
        if (this.jsonData && this.jsonData.PhysicalAssessment && this.jsonData.PhysicalAssessment.HC_CHA_OTBA_SensoryObservation) {
            let BodyPartAssessmentData = this.jsonData.PhysicalAssessment.HC_CHA_OTBA_SensoryObservation;
            ...
            this.bodyPartSelections = BodyPartAssessmentData
                .filter(item => item.HC_CHA_OTBA_LocationofSharpDul_discrimination)
                .map(item => ({
                    HC_CHA_OTBA_LocationofSharpDul_discrimination: item.HC_CHA_OTBA_LocationofSharpDul_discrimination,
                    HC_CHA_OTBA_SharpDulldiscriminationfinding: item.HC_CHA_OTBA_SharpDulldiscriminationfinding,
                    HC_CHA_OTBA_SpecifySharpDulldiscrimination:item.HC_CHA_OTBA_SpecifySharpDulldiscrimination                    
                }));
        }        
        
        if(this.jsonData && this.jsonData.PhysicalAssessment && this.jsonData.PhysicalAssessment.Block28 && this.jsonData.PhysicalAssessment.Block28.HC_CHA_OTBA_LocationofSharpDul_discriminationFormulaResult)
        {
            const formulaValue = this.jsonData.PhysicalAssessment.Block28.HC_CHA_OTBA_LocationofSharpDul_discriminationFormulaResult;
            if (formulaValue==1) {            
    ...
    isCircleBlue(circle) {
        return this.bodyPartSelections.some(selection => selection.HC_CHA_OTBA_LocationofSharpDul_discrimination === circle['data-value']);
    }
        ...
        this.circles.forEach(circle => {
            if (circle.selected) {
                ...
                const matchedSelection = this.bodyPartSelections.find(selection =>
                    selection.HC_CHA_OTBA_LocationofSharpDul_discrimination === circle['data-value']
                );

                if (isTooltip && this.tooltipVisible && this.tooltipText === circle['data-value']) {
                    const tooltipLocation = circle['data-value'];
                    const tooltipProperties1 = `Sharp/Dull discrimination finding  ${matchedSelection.HC_CHA_OTBA_SharpDulldiscriminationfinding}`;
                    const tooltipProperties2 = `Specify Sharp/Dull discrimination  ${matchedSelection.HC_CHA_OTBA_SpecifySharpDulldiscrimination}`;
